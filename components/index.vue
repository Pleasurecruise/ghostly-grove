<template>
  <div class="home">
    <div class="left">
      <!-- 用户信息 -->
      <div class="user" @click="setting.select = [true]">
        <div class="avatar">
          <img :src="user.avatar" alt="" class="avatar-image"/>
          <img src="../assets/images/badge.webp" alt="" class="badge"/>
        </div>
        <div class="name">{{ user.name }}</div>
      </div>
      <!-- 标签列表 -->
      <div class="label-list">
        <div class="label" :class="{ highlight: label === '' }" @click="label = ''">
          <div class="icon"></div>
      ...
      <!-- 发布新帖 -->
      <div class="menu" @click="onBtnClick">
        <img src="../assets/images/ghost.webp" alt="" class="icon"/>
...
const labelList = computed(() => {
  ...
  message.list.forEach((msg) => {
    const res = msg.text.match(/#\S+?#/g)
    res?.forEach((name) => {
      if (list.has(name)) {
        list.get(name)?.push(msg)
      } else {
        list.set(name, [msg])
      }
    })
